    ...
    <div className="p-6">
      <Card className="p-6 shadow">
        ...
        <div className="border p-4 rounded mb-6 text-sm">
          <div>
            <b>Đơn vị bán hàng:</b> {clinicInfo?.name}
          </div>
          <div>
            <b>Mã số thuế:</b> {clinicInfo?.taxCode}
          </div>
          <div>
            <b>Địa chỉ:</b> {clinicInfo?.address?.street} -{" "}
            {clinicInfo?.address?.districtName} - {clinicInfo?.address?.city}
          </div>
          <div>
            <b>SĐT:</b> {clinicInfo?.phone} &nbsp; <b>Website:</b>{" "}
            {clinicInfo?.website}
          </div>
          <div>
            <b>Số tài khoản:</b> {clinicInfo?.accountNumber} — <b>Ngân hàng:</b>{" "}
            {clinicInfo?.bankName}
          </div>
        </div>
        ...
        <div className="border p-4 rounded mb-6 text-sm">
          <div>
            <b>Tên khách hàng:</b> {patient?.fullName}
          </div>
          <div>
            <b>Mã bệnh nhân:</b> {patient?.patientCode}
          </div>
          <div>
            <b>Địa chỉ:</b> {patient?.address}
          </div>
          <div>
            <b>Hình thức thanh toán:</b> TM
          </div>
        </div>
        ...
        {user?.staff && (
          <div className="mt-6 flex justify-end">
            ...
            {invoice.assignedStaff?.id === user.staff.id && !invoice.paid && (
              <Button
                size="sm"
                variant="secondary"
